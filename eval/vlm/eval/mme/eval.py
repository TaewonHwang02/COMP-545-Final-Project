...
if __name__ == '__main__':
    ...
    # added subsample argument parser
    ...
    for filename in os.listdir(args.root):
        ...
        lines = fin.readlines()
        # --- SUBSAMPLE MODE ---
        if hasattr(args, "subsample") and args.subsample > 0:
            import random
            random.seed(0)
            lines = random.sample(lines, min(args.subsample, len(lines)))
            print(f"[subsample] Using {len(lines)} samples out of {len(fin.readlines())}")
        # ----------------------
        ...
        for line in tqdm(lines):
            ...
            # correct path
            img_path = os.path.join('eval/vlm/eval/mme/MME_Benchmark_release_version', filename, img)
            if not os.path.exists(img_path):
                img_path = os.path.join('eval/vlm/eval/mme/MME_Benchmark_release_version', filename, "images", img)
            if not os.path.exists(img_path):
                continue
