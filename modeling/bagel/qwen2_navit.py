...
# for zero-ablation
def _zero_module_params(module: nn.Module):
    """
    In-place zero all parameters of a module.
    Used for zero-ablation of attention + MLP blocks.
    """
...
class Qwen2Model(Qwen2PreTrainedModel):
    def __init__(self, config):
        ...
        # make sure we only zero once
        self._zero_ablation_done = False

        # Initialize weights and apply final processing
        self.post_init()
    
    
    def zero_ablate_layer(self, layer_idx: int):
        
        if layer_idx < 0 or layer_idx >= len(self.layers):
            print(f"[ZeroAblation] Ignoring invalid layer index {layer_idx}")
            return

        layer = self.layers[layer_idx]

        # zero attention module
        if hasattr(layer, "self_attn"):
            _zero_module_params(layer.self_attn)

        # zero MLP(s)
        if hasattr(layer, "mlp"):
            _zero_module_params(layer.mlp)
        if hasattr(layer, "mlp_moe_gen"):
            _zero_module_params(layer.mlp_moe_gen)

        print(f"[ZeroAblation] Zeroed attention + MLP weights for layer {layer_idx}")
    ...
    def forward_inference(
        ...
        #  Zero-ablation config from env (run once per model)
        ablate_layer_env = os.getenv("ABLATE_LAYER", None)
        if not self._zero_ablation_done and ablate_layer_env is not None:
            try:
                ablate_layer_idx = int(ablate_layer_env)
                self.zero_ablate_layer(ablate_layer_idx)
            except ValueError:
                # ignore invalid env values
                pass
            self._zero_ablation_done = True
